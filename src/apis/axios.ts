...
// 전역 변수로 refresh요청의 promise를 저장해서 중복 요청 방지
...
// 응답 인터셉터: 401 에러 발생 시 refreshToken을 사용하여 accessToken 갱신
axiosInstance.interceptors.response.use(
  (response) => response, // 응답이 성공적이면 응답 데이터 반환
  async (error) => {
    const originalRequest: CustomInternalAxiosRequestConfig = error.config;
    // 401 에러면서, 아직 재시도 하지 않은 경우
    if (
      error.response &&
      ...
      if (!refreshPromise) {
        ...
        refreshPromise = (async () => {
          ...
          // 새 acacessToken을 반환하여 다른 요청들이 이것을 사용할 수 있게끔 
